...
//float2 结构，具有两个组件的向量，类型仅在 C++ 中可用
extern float2* aux_dev[6];
extern float2* aux_host_1[6];
extern float2* aux_host_2[6];


extern cudaStream_t compute_stream;//计算流
extern cudaStream_t h2d_stream;//host 2 device
extern cudaStream_t d2h_stream;//device 2 host
extern cudaEvent_t events[1000];//记录事件
